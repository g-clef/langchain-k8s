...
class AsyncDuckDuckGoSearchAPIWrapper(BaseModel):
    ...
    async def _ddgs_text(
        ...
        async with AsyncDDGS() as ddgs:
            ...
            if ddgs_gen:
                return [r async for r in ddgs_gen]
        return []
    ...
    async def results(
        ...
        if source == "text":
            results = [
                ...
                for r in await self._ddgs_text(query, max_results=max_results)
            ]
        elif source == "news":
            results = [
                {
                ...
                for r in await self._ddgs_news(query, max_results=max_results)
            ]
        else:
            results = []
